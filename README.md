...
- [x] **M1** — Vite + React + Tailwind + Caddy + Hono hello-world
- [x] **M2** — Supabase auth + notes table + plain CRUD API
- [x] **M3** — TanStack DB + Electric provider + notes collection + persistence + `useCollection` hook
- [x] **M4** — Service Worker + offline mutation queue (web)
- [x] **M5** — Capacitor iOS + Fastlane lanes + native auth/network
- [x] **M6** — iOS Capgo OTA pipeline (self-hosted)
- [x] **M7** — Electron main + preload + tray + IPC + electron-builder
- [x] **M8** — Electron shell auto-updater
- [x] **M9** — Electron renderer OTA
- [x] **M10** — Android + final docs
...
│   ├── main/updater.ts        electron-updater wrapper (M8)
│   ├── main/renderer-ota.ts   custom renderer OTA state machine (M9)
