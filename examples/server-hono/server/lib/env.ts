 ...
 *
 * M1 has no required vars. Later milestones add Supabase keys (M2),
 * Electric source URL (M3), and S3 credentials (M6/M8/M9). Add new
 * required vars here so misconfigured deployments fail fast at startup
 * instead of silently 500ing the first time the dependency is touched.
 */

interface RequiredVar {
  name: string;
  /** Set true once the milestone that needs this var lands. */
  required: boolean;
  hint: string;
}

const VARS: RequiredVar[] = [
  // M2+ (now required — auth/CRUD won't work without these)
  ...
  // M3+ (required — Electric proxy refuses to serve without these)
  ...
  // M6+ (OTA pipelines — required so /api/capacitor/bundle doesn't 500)
