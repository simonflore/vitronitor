...
- **M1**: Vite + React 19 + Tailwind v4 + Caddy HTTP/2 dev proxy + Hono
...
- **M2**: Supabase magic-link auth, server-side `withAuth` middleware
...
- **M3**: ElectricSQL + TanStack DB sync. Server-side proxy with org-scoped
  ...
  Zod schemas.
- **M4**: Real Service Worker (cache-first static, network-first /api,
...
- **M5**: Capacitor iOS scaffolding (config, native-storage adapter for
...
- **M6**: Self-hosted iOS Capgo OTA pipeline. RSA-signed bundles via
...
- **M7**: Electron main process (BrowserWindow, single-instance lock,
...
- **M8**: electron-updater wrapper (autoDownload=false, hourly check,
...
- **M9**: Custom renderer OTA state machine
...
- **M10**: Android target documented (same Capgo pipeline works).
